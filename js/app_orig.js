	...
	.controller("ktCtrl", function($scope, $rootScope) {
		...
		// ACCOUNT/SESSION
		{
			...
			$scope.initLogin = function() {
				...
				console.log("initLogin() - Current User: " + $scope.currentuser);
				...
				$scope.loginform.redirecturl = decodeURIComponent(GetQS("ru"));
				$scope.loading = false;
			}
			
			$scope.initSignup = function() {
				trackEvent('signup', 'form');
			}
			...
			$scope.logout = function() {
				...
				$.ajax({
					...
					error: function(data, status, error) {
						//[TBD]
						trackEvent('auth', 'logoutError');
					}
				});
			}
			
			$scope.initSession = function() {
				// Start at each page load - Get the current session (if any)
				console.log("initSession()");
				$scope.getSession();
			}
			
			$scope.getSession = function(su, er) {
				...
				$.ajax({
					...
					//	Not great practice, but we'll fix it later
					async: false,
					dataType: 'json',
					success: function(data) {
						$scope.currentuser = data;
		...
		// COMPENDIUM
		{
			...
			// Init loading all factions, teams, and operatives
